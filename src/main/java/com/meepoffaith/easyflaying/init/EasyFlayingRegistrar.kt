...
typealias RegistrarEntry<T> = EasyFlayingRegistrar<T>.Entry<out T>

// https://github.com/MEEPofFaith/hextra-patterns-1.21/blob/main/src/common/main/kotlin/com/meepoffaith/hextrapats/registry/HextrapatsRegistrar.kt
abstract class EasyFlayingRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(EasyFlaying.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is EasyFlayingRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
